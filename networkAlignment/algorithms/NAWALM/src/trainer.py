...
from algorithms.MUSE.src.utils import get_optimizer, load_embeddings, normalize_embeddings, export_embeddings
from algorithms.MUSE.src.utils import clip_parameters
from algorithms.MUSE.src.dico_builder import build_dictionary
from algorithms.MUSE.src.evaluation.word_translation import DIC_EVAL_PATH, load_identical_char_dico, load_dictionary
import numpy as np
...
class Trainer(object):
    ...
    def __init__(self, src_emb, tgt_emb, mapping, discriminator, params, \
                 source_deg = None, target_deg = None, p=None, num=None, allgt=None):
        """
        ...
        if hasattr(params, 'map_optimizer'):
            ...
            #self.map_optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, mapping.parameters()), lr=0.01)
            self.map_optimizer = optim_fn(mapping.parameters(), **optim_params)
        if hasattr(params, 'dis_optimizer') and params.dico_train == "default":
    ...
    def get_dis_xy(self, volatile):
        ...
        bs = self.params.batch_size  # 32
        mf = self.params.dis_most_frequent
        assert mf <= min(len(self.src_dico), len(self.tgt_dico))

        # if self.source_deg is not None:
        arg_sort_source = np.argsort(self.source_deg)
        arg_sort_target = np.argsort(self.target_deg)
    ...
    def mapping_step(self, stats):
        ...
        if self.dico is not None:
            source_ids = self.dico[:, 0]
            target_ids = self.dico[:, 1]
            with torch.no_grad():
                src_emb = self.src_emb(Variable(source_ids))
                tgt_emb = self.tgt_emb(Variable(target_ids))
            src_emb = self.mapping(Variable(src_emb.data))
            tgt_emb = Variable(tgt_emb.data)
            weakly_loss = torch.sum((src_emb - tgt_emb)**2) / len(self.dico)
            loss = weakly_loss + loss
    ...
    def save_best(self, to_log, metric):
        ...
        if to_log[metric] > self.best_valid_metric:
            ...
            # save the mapping
            W = self.mapping.layer.weight.data.cpu().numpy()
