...
class NAWAL(NetworkAlignmentModel):
    ...
    def align(self):
        if self.args.load_emb:
            self.source_embedding = read_embedding_from_file(self.source_dataset.id2idx, \
                f'{self.path}/outs/{self.args.embedding_name}/emb_source', self.args.embedding_dim, self.args.cuda)
            self.target_embedding = read_embedding_from_file(self.target_dataset.id2idx, \
                f'{self.path}/outs/{self.args.embedding_name}/emb_target', self.args.embedding_dim, self.args.cuda)
        else:
            self.learn_embeddings()

            if self.args.save_emb:
                #'algorithms/NAWAL/embeddings'
                to_word2vec_format(self.source_embedding, self.source_dataset.G.nodes(), f'{self.path}/outs/{self.args.folder}', self.args.embedding_name + "_source", \
                    self.args.embedding_dim, self.source_dataset.id2idx)
                to_word2vec_format(self.target_embedding, self.target_dataset.G.nodes(), f'{self.path}/outs/{self.args.folder}', self.args.embedding_name + "_target", \
                    self.args.embedding_dim, self.target_dataset.id2idx)

        if not self.args.NAWAL_only:
    ...
    def nawal_mapping(self):
        ...
        for n_iter in range(self.n_refinement):
            ...
            dico = build_dictionary(src_emb, tgt_emb, num_keep = self.args.num_keep)
